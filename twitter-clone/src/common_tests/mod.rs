...
#[allow(unused)]
pub async fn get_app_state<T: Debug>(db_repo: T) -> AppState<T> {
    AppState { client: reqwest::Client::new(), db_repo }
}

pub async fn get_app_data<T: Debug>(db_repo: T) -> web::Data<AppState<T>> {
